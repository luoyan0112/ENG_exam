...
from .config import DB_CONFIG, CURRENT_SUBJECT
...
def get_subject_id():
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id FROM subjects WHERE name = %s", (CURRENT_SUBJECT,))
    row = cur.fetchone()
    cur.close()
    conn.close()
...
def get_random_questions(count=None, question_type=None):
    ...
    try:
        ...
        standalone_types = {'writing', 'translation'}
...
def get_user_stats(user_id):
    ...
    try:
        ...
        cur.execute("""
            ...
            FROM practice_records WHERE user_id = %s
        """, (user_id,))
        row = cur.fetchone()
