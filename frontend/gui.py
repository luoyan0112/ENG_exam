...
class LoginWindow:
    ...
    def _build_ui(self):
        ...
        tk.Label(title_frame, text='英语刷题系统', font=FONT_TITLE,
...
class PracticePanel:
    ...
    def _build_ui(self):
        ...
        type_combo = ttk.Combobox(control_frame, textvariable=self.type_var,
                                  values=['全部', '写作', '仔细阅读', '选词填空', '长篇阅读', '翻译'],
    ...
    def _show_welcome(self):
        """显示欢迎信息"""
        self._clear_content()
        tk.Label(self.scrollable_frame, text='欢迎使用英语刷题系统！',
    ...
    def _render_current_question(self):
        ...
        if item['is_composite']:
            ...
            if q['type'] == 'translation':
                tk.Label(answer_frame, text='请写出译文:', font=('Microsoft YaHei', 11, 'bold'),
                         fg=COLOR_PRIMARY, bg=COLOR_WHITE).pack(anchor='w')
                self.fill_entry = tk.Text(answer_frame, height=5, font=FONT_NORMAL,
    ...
    def _open_notebook(self):
        """打开共享笔记本"""
        NotebookWindow(self.parent, self.user)
    ...
    def _submit_answer(self):
        ...
        if q['type'] in ('writing', 'translation'):
            # ===== 主观题（写作/翻译）：分数制评价 =====
            max_score = q.get('score', 15)
            result_color = COLOR_WARNING
            result_text = '请参考范文并给自己评分'
    ...
    def _show_answer_feedback(self, q, user_answer):
        ...
        if q['type'] in ('writing', 'translation'):
            label_text = '【参考范文】' if q['type'] == 'writing' else '【参考译文】'
...
class WrongAnswerPanel:
    ...
    def _load_data(self):
        ...
        type_names = {
            'writing': '写作', 'careful_reading_question': '仔细阅读',
            'banked_cloze_blank': '选词填空', 'long_reading_question': '长篇阅读',
            'translation': '翻译',
        }
        ...
        for item in wrong_answers:
            ...
            tk.Button(btn_frame, text='重新练习', font=FONT_SMALL,
                      ...
                      command=lambda qid=item.get('question_id'): self._practice_question(qid)).pack(anchor='e')
...
class StatsPanel:
    ...
    def _build_ui(self):
        ...
        tk.Label(title_frame, text='数据分析', font=FONT_TITLE,
                 fg=COLOR_PRIMARY, bg=COLOR_BG).pack(side='left', padx=10)
    ...
    def _load_data(self):
        ...
        cards = [
            ('练习次数', str(stats.get('total_practices', 0)), COLOR_ACCENT),
        ...
        if not stats.get('total_practices'):
...
class PracticeDialog(tk.Toplevel):
    ...
    def _load_question(self):
        ...
        elif q['type'] in ('writing', 'translation'):
            self.answer_text = tk.Text(answer_frame, height=6, font=FONT_NORMAL,
                                       bd=2, relief='groove', wrap='word')
...
class NotebookWindow(tk.Toplevel):
    ...
    def __init__(self, parent, user):
        ...
        self.pages = []
        self.current_idx = 0
        self._drawing_mode = False       # 是否处于画笔模式
        self._drawing_strokes = []       # 本次绘制的笔画 [{points, color, width}]
        self._current_pts = []           # 当前正在画的点
    ...
    def _format_content(self, text, overlays):
        """将 text + overlays 格式化为存储用的字符串"""
        if not overlays:
            return text  # 无叠加层时保持纯文本格式（向后兼容）
        return json.dumps({'text': text, 'overlays': overlays}, ensure_ascii=False)

    def _render_text_content(self, page, y_offset, cw):
        """渲染文字内容"""
        text, _ = self._parse_content(page)
    ...
    def _render_overlays(self, page):
        ...
        _, overlays = self._parse_content(page)
        if not overlays:
            return
    ...
    def _adj_canvas_middle(self, event):
        ...
        if not messagebox.askyesno('确认删除', f'确定要删除截图叠加层 {idx+1} 吗？'):
            return
        page = self.pages[self.current_idx]
        text, overlays = self._parse_content(page)
        if idx >= len(overlays):
            return
        # 清理图片文件
        ...
        if old_path and os.path.exists(old_path):
            try:
                ...
                pass
        overlays.pop(idx)
        new_content = self._format_content(text, overlays)
    ...
    def _add_text_page(self):
        ...
        ok, _, msg = db_module.add_notebook_page(
            self.user['id'], content='', note_type='text')
        if ok:
            self._load_data()
    ...
    def _add_screenshot(self, hide_window=True):
        ...
        try:
            ...
            cw = self.page_canvas.winfo_width() or 600
            default_x = 30
            default_y = 60 + len(overlays) * 30  # 多层叠加时错开位置
            overlays.append({
                'image_path': filepath,
                'x': default_x,
                'y': default_y,
...
class MainApplication:
    ...
    def _build_ui(self):
        ...
        tk.Label(top_bar, text='英语刷题系统', font=('Microsoft YaHei', 14, 'bold'),
                 fg=COLOR_WHITE, bg=COLOR_PRIMARY).pack(side='left', padx=20, pady=10)
        ...
        # 刷题面板
        practice_frame = tk.Frame(notebook, bg=COLOR_BG)
        notebook.add(practice_frame, text='  刷题  ')
        self.practice_panel = PracticePanel(practice_frame, self.user)

        # 错题本面板
        wrong_frame = tk.Frame(notebook, bg=COLOR_BG)
        notebook.add(wrong_frame, text='  错题本  ')
        WrongAnswerPanel(wrong_frame, self.user)

        # 笔记本面板
        note_frame = tk.Frame(notebook, bg=COLOR_BG)
        notebook.add(note_frame, text='  笔记本  ')
        NotesPanel(note_frame, self.user)

        # 数据分析面板
        stats_frame = tk.Frame(notebook, bg=COLOR_BG)
        notebook.add(stats_frame, text='  数据分析  ')
        StatsPanel(stats_frame, self.user)
